...
import           Database.Persist (get, insert, delete, entityVal, Entity(..), getBy, selectList, SelectOpt(..))
import           Database.Persist.Sql (fromSqlKey, toSqlKey)
import           Database.Persist.Sqlite (withSqliteConn, runMigration, SqlPersistT, Entity)
...
localConnString = "roboticsWebsite.sqlite3"--"host=127.0.0.1 port=5432 user=postgres dbname=postgres"
...
runAction connectionString action =
  runStdoutLoggingT $ filterLogger logFilter $ withSqliteConn connectionString $ \backend ->
...
-- getContent :: Int -> DB (Maybe Year)
getContentByYear connString year = runAction connString $ do
    content <- getBy $ Date year --selectList [YearDate ==. (coerce year)] [] --[OffsetBy $ coerce year]
    case content of
        Just (Entity _ record) -> pure $ Just record
        _ -> pure Nothing

getContentDef connString = runAction connString $ do
  content <- selectList [] [LimitTo 1]
  case content of
    [Entity _ (Year _ endyear)] -> pure endyear
    _ -> pure undefined

getCustomByName connString name = runAction connString $ do
  custom <- getBy $ Name name
  case custom of
    Just (Entity _ customPage) -> pure $ Just customPage
    _ -> pure Nothing
